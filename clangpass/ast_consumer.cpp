...
replace_pass::CodeRefactorASTConsumer::CodeRefactorASTConsumer(
    ...
    for (auto name: names) {
      ...
      const auto qualified_matcher = qualifiedTypeLoc(
        hasUnqualifiedLoc(
          elaborated_matcher
        )
      );
      match_finder.addMatcher(elaborated_matcher.bind("ElaboratedTypeLoc" + name.old_name), &refactor_handler);
      match_finder.addMatcher(qualified_matcher.bind("QualifiedTypeLoc" + name.old_name), &refactor_handler);
    }
  }

std::string replace_pass::CodeRefactorASTConsumer::RefactoredFileName(StringRef original_filename) const {
  ...
  std::string path = std::string(original_filename.begin(), original_filename.begin() + slash_index);
  std::string filename = std::string(original_filename.begin() + slash_index, original_filename.end());
