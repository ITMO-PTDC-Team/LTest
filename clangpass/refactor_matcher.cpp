  ...
  switch (type) {
    ...
    case TypeName::TemplateName: {
      if (const auto* ETL = result.Nodes.getNodeAs<ElaboratedTypeLoc>("ElaboratedTypeLoc" + p.old_name)) {      
        ...
        rewriter.ReplaceText(ETL->getSourceRange(), p.new_name + GetArgumentsFromTemplateType(TemplType));
      }
      if (const auto* QTL = result.Nodes.getNodeAs<QualifiedTypeLoc>("QualifiedTypeLoc" + p.old_name)) {
        ...
        rewriter.ReplaceText(QTL->getSourceRange(), p.new_name + GetArgumentsFromTemplateType(TemplType));
      }
    }
  }
}
...
std::string CodeRefactorMatcher::GetArgumentsFromTemplateType(const TemplateSpecializationType *type) {
  std::string args;
  llvm::raw_string_ostream os(args);
  printTemplateArgumentList(os, type->template_arguments(), context.getPrintingPolicy());
  return args;
}
