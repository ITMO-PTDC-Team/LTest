...
struct LinearizabilityDualCheckerRecursive final : DualModelChecker {
  ...
  bool Check(const std::vector<DualHistoryEvent>& history) override {
    ...
    // A completed request without FollowUpResponse remains pending in the
    // checked history. At the end we probe its follow-up on a copy of the spec
    // state: if it is ready, this linearization cannot explain the history.
    ...
    auto validate_completed_linearization =
        [this, &incomplete_followups](const SpecState& state) -> bool {
      for (const auto& pending : incomplete_followups) {
        ...
        SpecState probe = state;
        const auto& blocking =
            std::get<DualBlockingMethod<SpecState>>(mit->second);
        if (blocking.followup(&probe, pending.args, pending.op_id).has_value()) {
          return false;
        }
      }
