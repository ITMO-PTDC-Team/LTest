...
int Run(int argc, char *argv[], std::vector<CustomRound> custom_rounds = {}) {
  ...
  size_t printer_threads = opts.threads;
  for (const auto& round : custom_rounds) {
    printer_threads = std::max(printer_threads, round.threads.size());
  }
  PrettyPrinter pretty_printer{printer_threads};
