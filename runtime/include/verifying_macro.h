...
auto ToStringList(const tuple_t &t) noexcept {
  ...
  constexpr auto s = std::tuple_size<TupleType>::value;
  if constexpr (s == 0) {
    return std::vector<std::string>{};
  }
  return ToStringListHelper<TupleType>(t, std::make_index_sequence<s>{});
}
...
#define DECLARE_TASK_METHOD(symbol) const char *symbol##_task_name = #symbol

#define TARGET_METHOD(gen, ret, cls, symbol, ...)          \
  DECLARE_TASK_METHOD(symbol);                             \
