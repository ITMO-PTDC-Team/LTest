...
//
// x starts at 3. A applies fetch_xor(1, relaxed) -> 2, then y.store(1,
// relaxed). B loads y then x and forbids y==1 with x still 3 (unchanged from
// B's view).
//
...
using spec_t = ltest::Spec<Exp26Test, LitmusTwoThreadsSpec, LinearWmmHash,
                           LinearWmmEquals>;
